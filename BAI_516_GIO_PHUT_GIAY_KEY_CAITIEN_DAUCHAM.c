...
void main()
{
   ...
   while(true)
   {
      ...
      s7seg.led[6] = m7d[gio%10]-gi; // Gi?i mã hàng don v? c?a giây
      s7seg.led[5] = 0xff-64;
      s7seg.led[4] = m7d[phut/10%10]; // Gi?i mã hàng ch?c c?a giây
      s7seg.led[3] = m7d[phut%10]-p; // Gi?i mã hàng don v? c?a giây
      s7seg.led[2] = 0xff-64;
      s7seg.led[1] = m7d[giay/10%10]; // Gi?i mã hàng ch?c c?a giây
      s7seg.led[0] = m7d[giay%10]-g; // Gi?i mã hàng don v? c?a giây
